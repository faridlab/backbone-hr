//! Domain policy for Employee aggregate
//!
...
/// Domain policy for Employee — permits all operations (no business invariants).
///
/// To add domain rules, replace this alias with a struct implementing
/// `backbone_core::DomainPolicy<Employee>` in the `// <<< CUSTOM` zone.
pub type EmployeeDomainPolicy = PermitAllPolicy<Employee>;
