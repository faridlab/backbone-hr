...
metaphor make entity Vendor --module hr
...
metaphor schema schema validate hr

# 3. Generate all artifacts (entity, DTOs, repo, service, handler, routes).
metaphor schema schema generate hr --target all --force

# 4. Generate the migration for the new entity.
metaphor migration generate Vendor hr
...
> `hr` is your module name (auto-detected from the current directory when omitted).
...
2. `metaphor schema schema validate hr`.
3. Generate a migration for the change:
   `metaphor migration generate <Entity> hr` (or, for a schema-diff-driven migration against
   a live DB, `metaphor schema schema migration hr --database-url …`).
4. Regenerate code: `metaphor schema schema generate hr --target all --force`.
