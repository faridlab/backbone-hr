...
metaphor migration generate Invoice hr
metaphor migration run
...
(`hr` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed hr          # run Rust seeders
metaphor migration generate-seeds hr  # emit SQL seed files
```
