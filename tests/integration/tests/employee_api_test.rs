//! Employee API Integration Tests
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Tests the Employee CRUD API endpoints.

use chrono::Utc;
use serde_json::{json, Value};
use uuid::Uuid;

use super::crud_test_base::{CrudTestConfig, GenericCrudTest, TestDataGenerator};
use crate::integration::framework::ApiTest;
use crate::integration::helpers::CommonUtils;

// ============================================================================
// EMPLOYEE TEST DATA GENERATOR
// ============================================================================

/// Test data generator for Employee
pub struct EmployeeTestData;

impl TestDataGenerator for EmployeeTestData {
    fn generate_create_payload(&self, _utils: &CommonUtils) -> Value {
        let now = Utc::now().to_rfc3339();
        json!({
            "id": Uuid::new_v4().to_string(),
            "company_id": Uuid::new_v4().to_string(),
            "employee_number": format!("Test {}", Uuid::new_v4().to_string().split('-').next().unwrap()),
            "user_id": null,
            "department_id": null,
            "first_name": format!("Test {}", Uuid::new_v4().to_string().split('-').next().unwrap()),
            "last_name": null,
            "designation": null,
            "employment_type": "permanent",
            "date_of_joining": now,
            "date_of_exit": null,
            "status": "active",
            "nik": null,
            "npwp": null,
            "tax_status": "tk0",
            "bank_account_no": null,
            "base_salary": 0,
            "metadata": json!({}),
        })
    }

    fn generate_update_payload(&self, id: &str, _utils: &CommonUtils) -> Value {
        let now = Utc::now().to_rfc3339();
        json!({
            "id": id,
            "company_id": Uuid::new_v4().to_string(),
            "employee_number": format!("Test {}", Uuid::new_v4().to_string().split('-').next().unwrap()),
            "user_id": null,
            "department_id": null,
            "first_name": format!("Test {}", Uuid::new_v4().to_string().split('-').next().unwrap()),
            "last_name": null,
            "designation": null,
            "employment_type": "permanent",
            "date_of_joining": now,
            "date_of_exit": null,
            "status": "active",
            "nik": null,
            "npwp": null,
            "tax_status": "tk0",
            "bank_account_no": null,
            "base_salary": 0,
            "metadata": json!({}),
        })
    }

    fn generate_invalid_payload(&self) -> Value {
        json!({
            // Missing required fields
        })
    }
}

// ============================================================================
// EMPLOYEE API TEST
// ============================================================================

/// Employee API test suite
pub struct EmployeeApiTest {
    inner: GenericCrudTest<EmployeeTestData>,
}

impl EmployeeApiTest {
    pub fn new() -> Self {
        let mut config = CrudTestConfig::new("/api/v1/employees", "Employee");
        config.supports_soft_delete = true;
        Self {
            inner: GenericCrudTest::new(config, EmployeeTestData),
        }
    }

    pub async fn run_all(&mut self) -> Vec<crate::integration::framework::TestResult> {
        self.inner.run_all().await
    }
}

impl Default for EmployeeApiTest {
    fn default() -> Self {
        Self::new()
    }
}

// ============================================================================
// TESTS
// ============================================================================

#[cfg(test)]
mod tests {
    use super::*;

    #[tokio::test]
    async fn test_employee_crud() {
        let mut test = EmployeeApiTest::new();
        let results = test.run_all().await;

        for result in &results {
            println!("{:?}", result);
        }

        // Check all tests passed
        let failed: Vec<_> = results.iter().filter(|r| !r.success).collect();
        assert!(failed.is_empty(), "Some tests failed: {:?}", failed);
    }
}

// ============================================================================
// CUSTOM TESTS
// ============================================================================

// <<< CUSTOM TESTS START >>>
// Add Employee specific tests here
// <<< CUSTOM TESTS END >>>
